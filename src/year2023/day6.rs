...
fn parse(input: &str) -> (Vec<u32>, Vec<u32>) {
    ...
    let time: Vec<u32> = re.find_iter(lines.next()
        .unwrap())
        .map(|c| c.as_str().parse::<u32>().unwrap())
        .collect();
    let distance: Vec<u32> = re.find_iter(lines.next()
        .unwrap())
        .map(|c| c.as_str().parse::<u32>().unwrap())
        .collect();
...
fn parse_pt2(input: &str) -> (u64, u64) {
    ...
    let time: u64 = re.find_iter(lines.next()
        .unwrap())
        .map(|c| c.as_str())
        .collect::<Vec<&str>>()
        .join("")
        .parse()
        .unwrap();
    let distance: u64 = re.find_iter(lines.next()
        .unwrap())
        .map(|c| c.as_str())
        .collect::<Vec<&str>>()
        .join("")
