...
impl Hand {
    ...
    fn sorted(&self) -> (Card, Card, Card, Card, Card) {
        let mut vals = [self.0, self.1, self.2, self.3, self.4];
        vals.sort_by_key(|k| -(k.value() as i32));
...
impl HandType {
    fn from_hand(h: Hand) -> Self {
        ...
        match (hand.0, hand.1, hand.2, hand.3, hand.4) {
            (a, b, c, d, e) if a == b && b == c && c == d && d == e => HandType::FiveOK,

            (a, b, c, d, e) if (b == c && c == d) && (a == b || d == e) => HandType::FourOK,

            (a, b, c, d, e) if a == b && (b == c || c == d) && d == e => HandType::FullHouse,

            (a, b, c, d, e) if (a == b && b == c) || (b == c && c == d) || (c == d && d == e) => {
                HandType::ThreeOK
            }

            (a, b, c, d, e) if (a == b && (c == d || d == e)) || (d == e && (b == c || c == d)) => {
                HandType::TwoPair
            }
            (a, b, c, d, e) if a == b || b == c || c == d || d == e => HandType::Pair,
...
impl PlayerHand {
    fn from_string(input: &str) -> Self {
        ...
        let (hand, bid) = (
            Hand::from_string(input[0], false),
...
fn parse(input: &str, part1: bool) -> Vec<(Hand, u32, HandType)> {
    input
        ...
        .map(|l| {
            let mut cb = l.split(' ');
            let (cards, bid): (Hand, u32) = (
                Hand::from_string(cb.next().unwrap(), part1),
                cb.next().unwrap().parse().unwrap(),
            );
...
fn parse_pt2(input: &str) -> Vec<PlayerHand> {
    input.lines().map(PlayerHand::from_string).collect()
}
